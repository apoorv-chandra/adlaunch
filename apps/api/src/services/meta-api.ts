...
// Wraps all Meta Graph API v18+ calls for campaign management
// ─────────────────────────────────────────────────────────────────────────────
...
export class MetaApiError extends Error {
  ...
  get isRateLimit(): boolean {
    return this.code === META_ERROR_CODES.RATE_LIMIT
  }

  get isTokenExpired(): boolean {
    return (
      this.code === META_ERROR_CODES.INVALID_ACCESS_TOKEN ||
      this.code === META_ERROR_CODES.TOKEN_EXPIRED
    )
  }
}
...
async function metaFetch<T>(
  ...
  const url =
    method === 'GET'
      ? `${META_GRAPH_BASE}${path}?${searchParams.toString()}`
      : `${META_GRAPH_BASE}${path}`
...
export async function createMetaCampaign(
  ...
  data: {
    name: string
    objective: string     // Our internal objective
  ...
  if (data.lifetimeBudgetCents) {
    body['lifetime_budget'] = String(data.lifetimeBudgetCents)
  }
  if (data.startTime) {
    body['start_time'] = data.startTime.toISOString()
  }
  if (data.endTime) {
    body['stop_time'] = data.endTime.toISOString()
  }
...
export interface CreateMetaAdSetData {
  ...
  objective: string           // Our internal objective (to derive optimization goal)
  dailyBudgetCents?: number
  targeting: {
    locations: string[]       // Country codes e.g. ['US', 'GB']
    ageMin: number
    ageMax: number
    genders: string[]         // Our: 'all'|'male'|'female'
    interests: string[]       // Meta interest IDs
    behaviors: string[]       // Meta behavior IDs
    customAudiences?: string[] // Meta custom audience IDs
  }
  startTime?: Date
  endTime?: Date
  placements?: object
...
export async function createMetaAdSet(
  ...
  const targeting: Record<string, unknown> = {
    geo_locations: {
      countries: data.targeting.locations.length > 0
        ? data.targeting.locations
        : ['US'],
    },
  ...
  if (data.targeting.interests.length > 0) {
    targeting['flexible_spec'] = [
      { interests: data.targeting.interests.map((id) => ({ id })) },
    ]
  }

  if (data.targeting.behaviors.length > 0) {
    targeting['behaviors'] = data.targeting.behaviors.map((id) => ({ id }))
  }

  if (data.targeting.customAudiences && data.targeting.customAudiences.length > 0) {
    targeting['custom_audiences'] = data.targeting.customAudiences.map((id) => ({ id }))
  }
  ...
  if (data.endTime) {
    body['end_time'] = data.endTime.toISOString()
  }
...
export async function updateMetaAdSet(
  adSetId: string,
  accessToken: string,
  data: { name?: string; status?: string; dailyBudgetCents?: number }
): Promise<{ success: boolean }> {
  const body: Record<string, unknown> = {}
  if (data.name) body['name'] = data.name
  if (data.status) body['status'] = STATUS_TO_META[data.status] ?? 'PAUSED'
...
export async function listMetaAdSets(
  campaignId: string,
  accessToken: string
): Promise<{ data: MetaAdSet[] }> {
  return metaFetch<{ data: MetaAdSet[] }>(
    `/${campaignId}/adsets`,
    {
      accessToken,
      params: {
        fields: 'id,name,campaign_id,status,effective_status,daily_budget,targeting,optimization_goal,billing_event,created_time,updated_time',
      },
    }
  )
}
...
export interface CreateMetaAdCreativeData {
  name: string
  pageId: string           // Facebook Page ID (required)
  imageUrl: string         // Public URL of creative image (R2 CDN URL)
  headline: string
  description?: string
  callToAction: string     // Our CTA e.g. 'SHOP_NOW'
...
export async function updateMetaAdStatus(
  ...
  return metaFetch<{ success: boolean }>(
    `/${adId}`,
    { method: 'POST', body: { status }, accessToken }
  )
}

// ── Campaign Publish (full flow) ──────────────────────────────────────────────
...
export async function publishFullCampaignToMeta(
  adAccountId: string,
  accessToken: string,
  payload: {
    campaign: {
    ...
    adSet: {
      name: string
      dailyBudgetCents?: number
      targeting: CreateMetaAdSetData['targeting']
      startTime?: Date
      endTime?: Date
    }
    creative: CreateMetaAdCreativeData
    ad: {
      name: string
    }
  }
  ...
  const { id: metaAdCreativeId } = await createMetaAdCreative(
    adAccountId,
    accessToken,
    payload.creative
  )
  ...
  // Step 4: Create Ad
  const { id: metaAdId } = await createMetaAd(
    adAccountId,
    accessToken,
    {
...
export async function getCampaignInsights(
  ...
  const result = await metaFetch<{ data: MetaInsights[] }>(
    `/${campaignId}/insights`,
    {
      accessToken,
      params: {
        fields: 'impressions,clicks,spend,reach,frequency,cpc,cpm,ctr,actions,action_values,date_start,date_stop',
...
// ── Sync Campaign Status from Meta ────────────────────────────────────────────

export async function syncCampaignStatusFromMeta(
  campaignId: string,
  accessToken: string
): Promise<{ internalStatus: string; metaStatus: string; effectiveStatus: string }> {
  const campaign = await getMetaCampaign(campaignId, accessToken)

  return {
    internalStatus: META_STATUS_TO_INTERNAL[campaign.effective_status] ?? 'paused',
