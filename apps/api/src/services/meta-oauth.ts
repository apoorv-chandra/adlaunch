...
export async function exchangeCodeForToken(config: MetaOAuthConfig, code: string): Promise<MetaTokenResponse> {
  const params = new URLSearchParams({
    client_id: config.appId,
  ...
  if (!res.ok) {
    ...
    throw new Error(`Meta OAuth exchange failed: ${err.error?.message ?? 'Unknown error'}`)
  }
  return res.json() as Promise<MetaTokenResponse>
}

export async function getLongLivedToken(config: MetaOAuthConfig, shortLivedToken: string): Promise<MetaTokenResponse> {
  const params = new URLSearchParams({
  ...
  if (!res.ok) {
    ...
    throw new Error(`Long-lived token exchange failed: ${err.error?.message ?? 'Unknown error'}`)
  }
  return res.json() as Promise<MetaTokenResponse>
}
...
export async function getMetaAdAccounts(accessToken: string): Promise<MetaAdAccount[]> {
  ...
  if (!res.ok) {
    const err = await res.json() as { error?: { message: string } }
    throw new Error(`Failed to fetch ad accounts: ${err.error?.message ?? 'Unknown error'}`)
  }
...
export async function verifyMetaToken(
  ...
  const data = await res.json() as { data: { is_valid: boolean; expires_at?: number; user_id?: string } }
  return {
