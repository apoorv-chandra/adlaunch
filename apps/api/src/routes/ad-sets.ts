// ─────────────────────────────────────────────────────────────────────────────
// Ad Sets Routes — CRUD + publish to Meta
// ─────────────────────────────────────────────────────────────────────────────
...
// ── GET /ad-sets — list ad sets for campaign ──────────────────────────────────
...
router.get('/', async (c) => {
  ...
  if (!campaignId) {
    return c.json({ error: 'campaignId query param required', code: 'BAD_REQUEST', status: 400 }, 400)
  }

  const data = await db
    .select()
    .from(adSets)
    .where(and(eq(adSets.tenantId, tenantId), eq(adSets.campaignId, campaignId)))

  return c.json({ data })
})

// ── POST /ad-sets — create ad set ─────────────────────────────────────────────
...
router.post('/', zValidator('json', createAdSetSchema), async (c) => {
  ...
  const [campaign] = await db
    .select()
    .from(campaigns)
    .where(and(eq(campaigns.id, body.campaignId), eq(campaigns.tenantId, tenantId)))
    .limit(1)
  ...
  const [adSet] = await db
    ...
    .values({
      ...
      status: 'active',
    } as typeof adSets.$inferInsert)
    .returning()

  return c.json({ data: adSet }, 201)
})

// ── PATCH /ad-sets/:id — update ad set ────────────────────────────────────────
...
router.patch('/:id', zValidator('json', createAdSetSchema.partial()), async (c) => {
  ...
  const [existing] = await db
    .select()
    .from(adSets)
    .where(and(eq(adSets.id, id), eq(adSets.tenantId, tenantId)))
    .limit(1)

  if (!existing) {
    return c.json({ error: 'Ad set not found', code: 'NOT_FOUND', status: 404 }, 404)
  }

  const [updated] = await db
    .update(adSets)
    .set({ ...body, updatedAt: new Date() } as Partial<typeof adSets.$inferInsert>)
    .where(eq(adSets.id, id))
    .returning()

  // If already on Meta, sync the update
  const { tenant } = c
  if (existing.metaAdSetId && tenant.metaAccessToken) {
    try {
      await updateMetaAdSet(existing.metaAdSetId, tenant.metaAccessToken, {
        name: body.name,
        dailyBudgetCents: body.dailyBudget,
      })
    } catch (err) {
      console.warn('Failed to sync ad set update to Meta:', err)
    }
  }
...
// ── POST /ad-sets/:id/publish — push ad set to Meta ───────────────────────────
...
router.post('/:id/publish', async (c) => {
  ...
  if (!tenant.metaAccessToken || !tenant.metaAdAccountId) {
    return c.json(
      { error: 'Meta Ads account not connected', code: 'META_NOT_CONNECTED', status: 400 },
      400
    )
  }
  ...
  const [campaign] = await db
    .select()
    .from(campaigns)
    .where(eq(campaigns.id, adSet.campaignId))
    .limit(1)

  if (!campaign?.metaCampaignId) {
    return c.json(
      { error: 'Parent campaign not published to Meta yet', code: 'CAMPAIGN_NOT_PUBLISHED', status: 400 },
      400
    )
  }
  ...
  const { id: metaAdSetId } = await createMetaAdSet(
    ...
    tenant.metaAccessToken,
    {
      ...
      dailyBudgetCents: adSet.dailyBudget ?? undefined,
      targeting,
    }
  )
  ...
  const [updated] = await db
    .update(adSets)
    .set({ metaAdSetId, updatedAt: new Date() } as Partial<typeof adSets.$inferInsert>)
    .where(eq(adSets.id, id))
    .returning()
...
router.delete('/:id', async (c) => {
  ...
  await db
    .update(adSets)
    .set({ status: 'deleted', updatedAt: new Date() } as Partial<typeof adSets.$inferInsert>)
    .where(eq(adSets.id, id))
